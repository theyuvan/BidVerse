...
/**
 * Requires a valid "Authorization: Basic base64(email:password)" header on every STOMP
 * CONNECT and stashes the resolved caller in the WebSocket session so bid messages can be
 * checked against it (see AuctionSocketController), instead of trusting the buyerId in
 * the payload.
 */
@Component
